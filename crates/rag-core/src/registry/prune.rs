...
pub fn prune(vault: &Vault, opts: &PruneOptions) -> Result<PruneReport> {
    // Determine target predicate.
    let (clause, args): (&'static str, Vec<String>) = if opts.all_non_indexed {
        ("status != 'indexed'", vec![])
    ...
    let select_sql = format!("SELECT path, status FROM files WHERE {clause} ORDER BY path");
    ...
    if !opts.dry_run && report.removed > 0 {
        let delete_sql = format!("DELETE FROM files WHERE {clause}");
