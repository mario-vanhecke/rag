...
use rag_core::config::{keys, Config};
...
/// Parse a string value (from the CLI) into a JSON value, guided by what we
/// know about the key's expected type.
fn parse_value(key: &str, raw: &str) -> anyhow::Result<Value> {
    // Use the typed Error::Config so the CLI maps to exit code 5 (config error)
    // instead of the generic exit code 1.
    let def = keys::lookup(key).ok_or_else(|| {
        anyhow::Error::from(rag_core::Error::Config(format!(
            "unknown config key '{key}'"
        )))
    })?;
    // Try JSON first; fall back to coercion based on the default's type.
    ...
    let _ = def; // silence unused-var warning if no special case matched.
